...
class TrainableTransformer(LightningModule):
    ...
    def __init__(self, hparams: Namespace) -> None:
        ...
        self.transformer = Transformer(
            ...
            weight_noise=self.hpar.weight_noise,
        )
    ...
    def prepare_data(self) -> None:
        ...
        (self.train_dataset, self.val_dataset,) = ArithmeticDataset.splits(
            train_pct=self.hpar.train_data_pct,  # type: ignore
            operator=self.hpar.math_operator,  # type: ignore
            operand_length=self.hpar.operand_length,  # type: ignore
            data_dir=self.hpar.datadir,  # type: ignore
        )
    ...
    def train_dataloader(self) -> ArithmeticIterator:  # type: ignore
        ...
        iterator = ArithmeticIterator(
            self.train_dataset,
            device,
            batchsize_hint=self.hpar.batchsize,  # type: ignore
        )
    ...
    def _scheduler_lr(self, step: int) -> float:
        ...
        """
        max_lr = self.hpar.max_lr  # type: ignore
        min_lr = self.hpar.max_lr / 10  # type: ignore
        warmup_steps = self.hpar.warmup_steps  # type: ignore
        if not self.hpar.anneal_lr:
            ...
            if step <= warmup_steps:
                lr = (float(step) / max(warmup_steps, 1)) * max_lr
            elif step <= self.hpar.anneal_lr_steps + warmup_steps:
                effective_step = step - warmup_steps
                t = effective_step / self.hpar.anneal_lr_steps
    ...
    def configure_optimizers(self) -> Tuple[List[Any], List[Dict]]:
        ...
        optimizer = CustomAdamW(
            ...
            eps=1e-8,
            lr=1,
            weight_decay=self.hpar.weight_decay,
            noise_factor=self.hpar.noise_factor,
            weight_decay_form=self.hpar.weight_decay_kind,
        )
        # optimizer = SAM(
        #     self.parameters(),
    ...
    def _step(
        ...
        y_hat, attentions, values = self(
            x=x, save_activations=self.hpar.save_activations  # type: ignore
        ...
        with torch.no_grad():
           # print(y_hat_rhs.shape, y_rhs.shape)
           # print(y_hat_rhs, y_rhs)
    ...
    def _save_inputs(self, outputs: Dict, ds: str) -> None:
        ...
        :param train: True is this is a training batch, false otherwise
        """
        logdir = self.hpar.logdir + "/inputs/" + ds  # type: ignore
    ...
    def _save_activations(self, outputs: Dict, ds: str) -> None:
        ...
        output: Dict[str, Any] = {}
        if self.hpar.save_outputs:  # type: ignore
            y_hat_rhs = torch.cat([x["y_hat_rhs"] for x in outputs])
            output["y_hat_rhs"] = y_hat_rhs
        if self.hpar.save_activations:  # type: ignore
        ...
        if self.hpar.save_outputs or self.hpar.save_activations:  # type: ignore
            logdir = self.hpar.logdir + "/outputs/" + ds  # type: ignore
            os.makedirs(logdir, exist_ok=True)
    ...
    def training_epoch_end(self, outputs):
        ...
        if epoch_is_to_be_logged:
            ...
            if self.hpar.save_activations or self.hpar.save_outputs:
                if self.current_epoch == 0:
